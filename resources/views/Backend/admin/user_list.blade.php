                                ...
                                <tbody>
                                    @foreach ($users as $key => $item)
                                        <tr>
                                            ...
                                            <td>
                                                @can('Edit')
                                                    <a href="{{ route('user.password.reset', $item->id) }}"
                                                        class="btn btn-outline-info my-1">Reset Password</a> <br>
                                                @endcan

                                                @can('Edit')
                                                    <a href="{{ route('user.profile.edit', $item->id) }}"
                                                        class="btn btn-inverse-warning" title="Edit"><i
                                                            data-feather="edit"></i></a>
                                                @endcan

                                                @can('Delete')
                                                    <a href="{{ route('user.profile.delete', $item->id) }}"
