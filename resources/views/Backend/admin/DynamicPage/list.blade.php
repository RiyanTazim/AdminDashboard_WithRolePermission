    ...
    <div class="container py-5">
        <div class="card">
            <div class="card-header bg-dark text-white d-flex justify-content-between">
                <h4 class="mb-0">Dynamic Page List</h4>
                <a href="{{ route('dynamicpage.create') }}" class="btn btn-light btn-sm">Add New</a>
            </div>
            <div class="card-body">
                <div class="table-responsive">
                    <table id="dynamicPageTable" class="table table-bordered table-striped table-hover">
                        <thead>
                            <tr>
                                <th class="text-center" width="10%">SL</th>
                                <th class="text-center">Title</th>
                                <th class="text-center">Content (short)</th>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#dynamicPageTable').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('dynamicpage.getData') }}",
                columns: [{
                        data: 'DT_RowIndex',
                        name: 'DT_RowIndex',
        ...
        const deleteRouteBase = "{{ route('dynamicpage.delete', ':id') }}";
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        success: function(response) {
                            $('#dynamicPageTable').DataTable().ajax.reload();
